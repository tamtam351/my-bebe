...
import { StarIcon, CameraIcon, VinylIcon, NeonHeartIcon } from './MemoryObjects.jsx'
...
export default function LoveLetter({ onNext }) {
  ...
  return (
    <SceneTransition className="letter-scene" duration={1.4}>
      ...
      <div className="neon-heart" style={{ top: '8%', left: '10%', animationDelay: '0.2s' }}>
        <NeonHeartIcon size={28} />
      </div>
      <div className="neon-heart" style={{ top: '12%', right: '12%', animationDelay: '1s' }}>
        <NeonHeartIcon size={24} color="#536DFF" />
      </div>

      <div className="letter-paper anim-fade-in-up" style={{ position: 'relative' }}>
        ...
        {showObjects && (
          <>
            <div style={{ position: 'absolute', bottom: 10, left: 12, opacity: 0.75 }} className="anim-fade-in">
              <StarIcon size={9} color="#c6a96b" />
            </div>
            <span className="letter-signoff anim-fade-in" style={{ display: 'block' }}>
              — always ♡
            </span>
          </>
        )}
      ...
      {showObjects && (
        <>
          <div
            className="memory-object anim-fade-in"
            style={{ position: 'absolute', bottom: '14%', left: '6%', animationDelay: '0.3s' }}
          >
            <CameraIcon size={40} />
          </div>
          <div
            className="memory-object vinyl-record anim-fade-in"
            style={{ position: 'absolute', bottom: '12%', right: '8%', animationDelay: '0.6s' }}
          >
            <VinylIcon size={44} />
          </div>
        </>
      ...
      {showObjects && (
        <button
          ...
          style={{ background: 'transparent', border: 'none', marginTop: 28, position: 'relative', zIndex: 5 }}
        >
          continue ♡
        </button>
      )}
